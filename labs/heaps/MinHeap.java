...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	void decrease(int loc) {
		...
		T parent_value=array[parent].getValue();
		T child_value=array[where].getValue();
		int status = (parent_value.compareTo(child_value));
		ticker.tick(3);
		ticker.tick();
		if (status > 0 ) {
	...
	public void swap(int from,int to) {
		...
		Decreaser<T> temp0 =  array[from];
		Decreaser<T> temp1 =  array[to];
		array[from] = array[to];
		array[to] = temp0;
		array[from].loc=from;
		array[to].loc=to;
	...
	private void heapify(int where) {
		...
		if(parent<=size) {
			...
			ticker.tick();
			if(array[right_child] == null) {
				ticker.tick();
				if(array[parent].getValue().compareTo(array[left_child].getValue()) > 0) {
					ticker.tick(2);
			...
			ticker.tick(2);
			if(array[left_child].getValue().compareTo(array[right_child].getValue()) <= 0 && array[parent].getValue().compareTo(array[left_child].getValue()) > 0) {
				ticker.tick(2);
				swap(parent,left_child);
			...
			else if(array[right_child].getValue().compareTo(array[left_child].getValue()) < 0 &&  array[parent].getValue().compareTo(array[right_child].getValue()) > 0) {
